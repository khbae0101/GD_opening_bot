...
# ── 시상 설정 ─────────────────────────────────────
# 시상 항목 ON/OFF (9월 운영안 확정 전까지 금액 항목은 OFF)
LUCKY_ON     = False   # 럭키추첨(개인·매장)
CLEANWEEK_ON = False   # 클린위크 시상
...
"""
    ...
    ip18_block = ""
    if today_str <= IP18_EXCLUDE_UNTIL:
        ip18_block = """# 아이폰18 사전예약 제외 (매우 중요)
- 아이폰18 계열은 아직 출시 전 '사전예약'이므로 개통 건수로 절대 세지 마라.
- 다음 모델 표기는 모두 아이폰18이다(대소문자·띄어쓰기 무관):
  AIP18, AIP18P, AIP18PM, Aip18pm, 아18, 아18프로, 아18프맥,
  아이폰18, 아이폰18프로, 아이폰18프로맥스, 18프로, 18프맥
- "락인", "사전예약", "AIP18 사전예약", "프리세일즈", "Pre" 같은 머리말이 붙은 줄과
  그 아래 이어지는 아이폰18 개통 줄도 모두 제외한다.
- 단, 아이폰17 계열(AIP17, AIP17P, AIP17PM, 아17, 아이폰17 등)은 정상 개통이므로 반드시 센다.
  "17"과 "18"을 혼동하지 마라.
- 한 사람의 보고에 아이폰18 예약만 있고 다른 개통이 없으면 그 사람은 집계에서 빼라(0건).
...
def compute_result(counts):
    ...
    if LUCKY_ON:
        ...
        lucky_person = lucky_store = None
        lucky_prize = store_prize = 0
...
def write_csv(res):
    ...
    lucky_txt = (f"{res['lucky_person']}({res['lucky_prize']:,}원)"
                 if res.get("lucky_person") else "")
    store_txt = (f"{res['lucky_store']}({res['store_prize']:,}원)"
                 if res.get("lucky_store") else "")
    _upsert_csv(
