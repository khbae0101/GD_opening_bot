...
- 매장명은 줄여서 올라올 수 있어 (예: 무실→원주무실, 임당→강릉임당, 의로→의정부로데오,
  유천→강릉유천, 상계→상계역, 먹골→먹골역, 건대→건대입구역, 중계→중계아울렛, 구리→구리리맥스,
  덕계→양주덕계 등). 위 목록의 매장으로 매칭해.
- "양주"만 단독으로 쓰이면 옥정신도시(양주)와 양주덕계가 모두 양주라 헷갈리니 "(확인필요)"로 표시해.
...
def main():
    ...
    if not messages:
        print("가져온 메시지가 없습니다. (읽기 타이밍/24시간 경계 가능성) 게시하지 않습니다.")
        return
    # 정상적인 마감 시간이면 보고가 여러 건 쌓임. 지나치게 적으면 너무 일찍 읽었거나
    # 이미 읽혀버린 경우라, '전부 미등록'으로 잘못 게시하지 않도록 보류한다.
    if len(messages) < 5:
        print(f"수집 메시지가 {len(messages)}건으로 너무 적어 보류합니다. "
              "(읽기 타이밍 문제일 수 있음 - 게시 안 함)")
        return
