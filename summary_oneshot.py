...
RETRY_CODES = (500, 502, 503, 504, 429)   # 텔레그램 일시 장애 → 재시도 가능
...
def _retryable(resp):
    ...
    if resp is None:
        return True                       # 네트워크 예외
    try:
        return resp.json().get("error_code") in RETRY_CODES
    except Exception:
        return resp.status_code in RETRY_CODES


def send_photo(path, caption, retries=3):
    ...
    for i in range(1, retries + 1):
        r = None
        try:
            with open(path, "rb") as f:     # 재시도마다 파일을 다시 연다
                r = requests.post(f"{API}/sendPhoto",
                                  data={"chat_id": TARGET_CHAT_ID, "caption": caption},
                                  files={"photo": f}, timeout=(10, 120))
        except Exception as exc:
            print(f"[출근] sendPhoto 전송 예외({i}/{retries}): {exc!r}")
        if r is not None and _report("sendPhoto", r):
            if i > 1:
                print(f"[출근] 재시도 {i}회차 성공")
            return True
        if i < retries and _retryable(r):
            wait = 10 * i
            print(f"[출근] {wait}초 후 재시도합니다")
            _time.sleep(wait)
        else:
            break
    return False
...
def send_text(text, retries=3):
    ...
    for i in range(1, retries + 1):
        ...
        try:
            ...
            print(f"[출근] sendMessage 전송 예외({i}/{retries}): {exc!r}")
        if r is not None and _report("sendMessage", r):
            if i > 1:
                print(f"[출근] 재시도 {i}회차 성공")
            return True
        if i < retries and _retryable(r):
